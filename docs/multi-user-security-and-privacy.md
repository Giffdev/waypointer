...
- **BLOCKER:** Sharing is an explicit enable action for an unlisted URL, with a server-derived complete-map snapshot that has no product flight-count ceiling, a field/precision summary, a forwarding warning, and a “view shared map” preview. Callers cannot select or exclude flight IDs. Resource failures must reject the operation without truncating or replacing the last enabled snapshot.
