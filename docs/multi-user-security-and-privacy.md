...
- **BLOCKER:** Sharing is an explicit Share action for an unlisted URL, with a server-derived complete-map projection that has no product flight-count ceiling, a field/precision summary, and forwarding and recognizable-route warnings. Callers cannot select or exclude flight IDs. Resource failures must reject the request without truncating the shared map or presenting a stale map as current.
- **BLOCKER:** An enabled URL is a live view of the owner's complete current eligible map. Each public read resolves the handle to an owner only when the share is enabled, unrevoked, and on a live account, then derives the projection from that owner's current flights, flight stops, and airports inside a read-only owner-scoped transaction; row-level security fails closed if that scope is ever missing. Newly imported, edited, enriched, and deleted flights are therefore reflected on the next read with no owner action. Owner-flight and route-stop mutations no longer disable the share: the frozen-snapshot invalidation triggers were removed in `0019_live_shared_maps` because a live view cannot disagree with the map it is derived from. Revocation stays explicit — disable, rename, account disablement, and deletion, and nothing else, remove public access.
...
- **BLOCKER:** Sharing is off by default. Enabling it records an explicit opt-in and nothing more; every read then derives a separate, explicitly redacted server-side projection over the owner's complete current eligible flight set, with redacted values/precision and a projection version. It must not change the visibility of source flight rows. `map_shares.projection` and `map_share_flights` are retained only as rollback artefacts and are never read by the live path.
...
- **BLOCKER:** The Share action is explicit consent to share the complete map projection — as it stands now and as it changes — including canonical airport names, identifiers, public reference locations, filter facts, and registration/tail number. The owner surfaces state that the map stays current until sharing is disabled. Before it is available, the UI discloses that content and that the user-chosen, unverified username is visible and forwardable. Generated OAuth/Firebase or migration usernames must be edited first and email is never exposed as a fallback. Disablement and renaming immediately make the current route unavailable.
...
- Forwarding-risk warning, disclosed airport/date/tail precision, automatic whole-map coverage, live reflection of newly imported/edited/enriched/deleted flights at an unchanged URL, and explicit-only revocation are verified.
...
- projection tests proving every current eligible flight is included automatically, that flights and route stops added, edited, or deleted after enabling appear at the same handle on the next read without a republish, that such a mutation does not disable the share, that a disabled share and a second owner's data are unreachable under non-superuser row-level security, that public airports use real identifiers/metadata, and that private account/session/source/internal-ID fields remain excluded;
