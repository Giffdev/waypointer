...
The account and sharing implementation reference is in [`multi-user-accounts-and-sharing.md`](multi-user-accounts-and-sharing.md), and its mandatory security/privacy contract is in [`multi-user-security-and-privacy.md`](multi-user-security-and-privacy.md). Waypointer implements isolated accounts with usernames, Auth.js sign-in, and one explicitly enabled, revocable travel-map capability at `/shared/{publicId}#key={capabilityKey}`. The authenticated owner view remains private and may show exact airports; the public API reads a separately approved coarse server-side snapshot containing every eligible flight at review time. New imports wait for an owner-initiated complete-map update, and any selected flight or route-stop mutation conservatively disables the whole share until reapproval. Account deletion disables normal login and sharing immediately. Social graphs and friend-only audiences remain explicit non-goals.
...
- `map_shares`, `map_share_flights`: one opt-in capability URL, token generation/digest, redacted projection, and server-derived approved snapshot membership; source flights never inherit share access.
...
- Authenticated owner contracts and capability shared contracts are separate allowlists. Owner views may show exact airports; the implemented shared DTO contains only an optional display name, counts, flight kind, country, and one-decimal coarse route coordinates. Exact-airport sharing is not implemented.
- Each preview automatically covers the complete eligible owner map, capped at 500 flights. Enabled snapshots do not auto-include newly added/imported flights. Owner-flight and route-stop mutations share canonical UUID-keyed PostgreSQL transaction locks with enablement, so an overlapping mutation cannot let stale consent publish. Any mutation touching a selected flight membership disables the whole share until explicit reapproval, including owner-only field edits; membership rows are retained until a later complete-map publish replaces them.
