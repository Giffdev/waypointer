...
The current sharing contract is in [`map-sharing-api.md`](map-sharing-api.md). Waypointer implements isolated accounts with usernames, Auth.js sign-in, and one explicitly enabled public travel map at `/{username}`. The URL is intentionally enumerable and has no token, GUID, fragment, or legacy compatibility route. Email is never exposed as a fallback. The authenticated owner view remains private; the Share action turns on a live, field-allowlisted server-side projection of every eligible flight the owner currently has, canonical public airport metadata, and only the flight facts required for viewer-local filters. New imports, edits, route-waypoint enrichment, and deletions appear at the same URL on the next read — there is no republish step, and an owner-flight or route-stop mutation no longer disables the share. Account deletion disables normal login and sharing immediately. Social graphs and friend-only audiences remain explicit non-goals.
...
- `map_shares`: one opt-in public username URL and its enabled/disabled state, which is the whole of what a share stores; source flights never inherit share access. Its `projection` column and the `map_share_flights` membership table are deprecated rollback artefacts, written when sharing is enabled and never read by the live public path.
...
- `flight_stops`: the ordered path of one flight. `stop_kind` is `landing` or `waypoint` and `source_field` is `endpoint`, `route`, or `manual`. **Only `landing` stops feed statistics, dedupe identity, and every count in the public share contract**; waypoints are presentation-only and are published as path geometry alone. Rows that predate this split migrate as `landing`/`endpoint`.
...
Because identity and statistics read landings only, adding, removing, or re-resolving waypoints across thousands of flights can never manufacture or collapse a flight. The private authenticated map draws them: `buildMapPageContract` sends `routePathFlights` — only the flights that actually carry a waypoint — and `FlightGlobe` renders them from two dedicated sources (`flight-map-route-paths`, `flight-map-route-waypoints`) into dedicated layers (`OVERFLOWN_LAYER_IDS`), dashed and hollow, beneath the flown routes. A flight with no `routePath` is absent from that payload and renders exactly the landing-only aggregate it always did. The route and airport sources are untouched, so route strength, mapped-airport counts, and the airport marker's "you have been here" meaning cannot move.

**The public shared map draws the same geometry, through the same code.** A shared map should look like the map its owner sees, so `PublicMapProjection["flights"][number].routePath` carries the same ordered landing/waypoint nodes, `derivePublicMapSlice` passes the filtered ones through, and `SharedMapView` hands `FlightGlobe` the same `MapRoutePathFlight[]` the private page does — there is no second rendering model. Visual parity is not semantic conflation: `routes`, `routeLegs`, the summary, and the published airport set are still built from landing stops alone, so a shared map's airport, route, and flight counts are identical whether or not a flight overflew somewhere. The path carries places and nothing else — no flight id, no raw route text, no source row — and every airport on the map, route endpoint and waypoint alike, is read from the same live catalog rows, so a code correction cannot make the same airport read two ways on one map. `routePath` is served only at contract=4: contract=3 was already shipped without it, and its exact-key parser rejects an unrecognised field outright, so `routePath` shipped as a new contract number rather than a new field on the existing one — see `PublicMapProjectionV3`/`toV3PublicMapProjection`.

**Shared maps are live views, not publications.** A public read resolves the handle to an owner through `public_share_owner_by_handle` — which answers only for an enabled, unrevoked share on a live account — and then derives the projection from that owner's current rows in a read-only, owner-scoped transaction. Nothing stored is consulted, so a flight imported, edited, enriched with waypoints, or deleted a moment ago is reflected on the next request at the same URL. `map_shares.projection` is still written when sharing is enabled, but only so a rolled-back build has a map to serve; it is never a fallback, because a stale map served as current is the failure this design removed. The schema-2 and schema-3 responses served to already-shipped browsers deliberately strip `routePath`, because those parsers reject unrecognised keys.
...
- `GET /api/import/resume` returns at most one batch: the newest that still owes its owner something — rows to review, an unfinished run, or a failure whose code the retry path accepts. Finished, deduplicated, cancelled, quarantined, and expired batches are excluded in SQL, and the query is `LIMIT 1`, so opening `/import` costs one row summary rather than one per batch ever imported. It replaced an unbounded `GET /api/import/batches`, and it keeps the retention sweep that entry point has always carried.
...
- Authenticated owner contracts and public shared contracts are separate allowlists. The public DTO (schema v4, carrying `routePath`, with the frozen pre-waypoint schema v3 shape served at `?contract=3` and a legacy v2 shape as the default for callers that omit `?contract`) includes canonical airport identifiers, names, cities, countries, facilities, and public reference coordinates plus date, kind, role, aircraft, registration, and per-leg route direction/references for viewer-local filtering. It excludes internal airport/flight/account/session IDs, exact times, notes, source/provenance, and import data — enforced by the public query selecting only allowlisted columns, not merely by shaping the response afterwards. Synthetic `R<number>` regions are never generated.
- Every public read covers the complete eligible owner map with no product flight-count ceiling, through exactly three bounded owner-scoped queries — flights, flight stops, and the airports those rows reference — so cost is flat in the number of flights and there is no per-flight query. Owner-scoped predicates keep the existing indices usable; PostgreSQL, memory/time, and response-size resources fail explicitly rather than producing a partial share. An enabled share auto-includes newly added, imported, edited, and enriched flights and drops deleted ones, because the projection is derived per request rather than stored. Enable/disable still take the canonical UUID-keyed PostgreSQL advisory lock they always did.
